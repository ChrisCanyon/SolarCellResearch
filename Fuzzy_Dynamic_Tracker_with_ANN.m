...
while(epochs<min_epochs)
    ...
    ANN_Max_idealV= ANN_V_MPP_Estimator(irrad,temp);
    Derivative_P_Over_V= P.del/V.del;
    
    ANN_Fuzzy_Input= ANN_Max_idealV-V.curr;
    
    %Fuzzy Control of change in voltage
    V.del= fuzzy_MPP_Controller_w_ANN(Derivative_P_Over_V,ANN_Fuzzy_Input);
...
plot(0:min_epochs/(length(Error.P)-1):min_epochs,abs(Error.P))
title('MFN MPPT - Error in Max Power')
xlabel('epochs')
...
ylabel('Watts')
hold on
title('MFN MPPT - Ideal and Actual Power Out of PV')
plot(Power.ideal, '.')
